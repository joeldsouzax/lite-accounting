/*
 * Filename: /Users/joel/Code/start-up/lite-accounting/app/home/transactions-list.tsx
 * Path: /Users/joel/Code/start-up/lite-accounting
 * Created Date: Friday, October 6th 2023, 4:47:09 pm
 * Author: Joel Dsouza
 *
...
const AccountsList: FC = () => {
  const {
    ...
    (index) => `/api/v1/transactions?page=${index}`,
    fetcher
  );
  ...
  if (error as Error) {
    return (
      <div className="alert alert-error">
        ...
        <span>Could not fetch transactions</span>
      </div>
    );
  }
    ...
    return (
      <div id="empty-accounts" className="card card-compact">
        <div className="card-body">
          <h2 className="card-title">no transactions found</h2>
        </div>
      </div>
    );
  }
        ...
        if (accounts.length < 1)
          return (
            <div className="alert" key="no-more-account">
              ...
              <span>no more transactions available</span>
            </div>
          );
        return (
          <div
          ...
          >
            {accounts.map(({ user_id, ...transaction }, i) => {
              return (
                <TransactionCard
                  {...transaction}
